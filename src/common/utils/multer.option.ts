...
export const multerAssetStockFactory = (
  ...
  return {
    storage: multerS3({
      ...
      key(_req: Express.Request, file, done) {
        ...
        const options: assetStockParams = _req['params'];

        if (!options.image_type) options.image_type = 'com';

        console.log(options);

        done(
          null,
          `assets/asset-stock/${options.image_type}/${Math.floor(
            Math.random() * 1001,
          )}_${Date.now()}${ext}`,
        );
      },
